...
def m2alen(request):
    if request.session.get("project"):
        ...
        role=request.session.get("role")
        today_date=date.today()
        return render(request,'m2alen.html',{'username':username,'role':role,'today_date':today_date}) 
...
def accounts(request):
    if request.session.get("email"):
        ...
        for action in dd_actions:
            dd=int(action.action_paid)
            all_price=all_price+dd
        return render(request,'accounts.html',{'project':project,'all_price':all_price,'username':username,'role':role})
    else:
...
def addpro(request):
    if request.method == 'POST':
        ...
        # username=request.session.get("username")
        # role=request.session.get("role")
        # return render(request,'today.html',{'username':username,'role':role,'items':items,'today_date':today_date,'all_items':ss_items,'all_price':all_price})
    ...
    if request.method == 'POST':
        ...
        if Project.objects.filter(name=pro_name).exists():
            ...
            for item in items:
                ss=int(item.item_total.encode('utf-8'))
                all_price=all_price+ss
            return render(request,'today.html',{'project':project,'username':username,'items':items,'role':role,'today_date':today_date,'all_price':all_price})       
...
def addaction(request,uid):
    if request.method == 'POST':
        ...
        i_paid=request.POST.get("paid")
        today_date=date.today()
        obj=Transaction(action_paid=i_paid,action_date=today,m2awel=m2awel_obj)
        obj.save()
